...
**入口职责**（`src/main.c`）：
- 初始化 `gpio_hal` 与 `display_hal`（若存在）
- 创建 `event_queue` / `event_loop` / `ui_thread`
- 监听信号（SIGINT/SIGTERM）并触发 `event_loop_request_shutdown()`
- 退出时按顺序回收资源（UI 线程 → event_loop → 队列 → HAL）
