...
**节拍说明**：主循环的最新节拍设计与实现要点见 `docs/architecture-main-tick.md`。

**相关架构决策**：
- 主循环节拍策略：见 ADR 0002
- ubus 阻塞 I/O 解耦：见 ADR 0003
- 多线程 UI 方案：见 ADR 0001（未来方案）

**技术债/注意事项**：
- 翻页动画当前为阻塞式，约 160ms 期间主循环暂停（后续可改为逐帧状态机）。
- `handle_button()` 内存在同步 I/O（如 ubus 操作），可能造成短暂卡顿（详见 ADR 0003）。
