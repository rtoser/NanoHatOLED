...
void page_controller_init(page_controller_t *pc, const page_t **pages, int page_count) {
    ...
    pc->current_page = (page_count > 1) ? 1 : 0;  /* Default to Network page */
    ...
    if (pc->page_mode == PAGE_MODE_VIEW) {
        switch (key) {
            ...
            case KEY_K2:
                if (long_press) {
                    return try_enter_mode(pc, now_ms);
                } else if (pc->current_page == 0) {
                    /* K2 short press on home page - screen off */
...
/* Forward declaration for u8g2 functions */
/* These will be provided by display_hal or linked directly */
extern void u8g2_SetFont(u8g2_t *u8g2, const void *font);
extern void u8g2_DrawStr(u8g2_t *u8g2, int x, int y, const char *str);
extern void u8g2_DrawHLine(u8g2_t *u8g2, int x, int y, int w);
...
extern void u8g2_SetClipWindow(u8g2_t *u8g2, int x0, int y0, int x1, int y1);
extern void u8g2_SetMaxClipWindow(u8g2_t *u8g2);

/* Font references - will be defined by display HAL */
extern const void *font_title;
...
static void render_title_bar(page_controller_t *pc, u8g2_t *u8g2,
    ...
    if (pc->page_mode == PAGE_MODE_VIEW && pc->anim.type != ANIM_ENTER_MODE) {
        ...
        if (page->can_enter) {
            /* Use simple arrow symbol */
            u8g2_DrawStr(u8g2, ENTER_IND_X + x_offset, PAGE_IND_Y, "<-");
        }
    }
...
static void render_page_content(page_controller_t *pc, u8g2_t *u8g2,
                                const sys_status_t *status, int page_idx, int x_offset) {
    if (page_idx < 0 || page_idx >= pc->page_count) return;
    ...
    page->render(u8g2, status, pc->page_mode);
...
void page_controller_render(page_controller_t *pc, u8g2_t *u8g2,
    ...
    if (pc->anim.type == ANIM_SLIDE_LEFT || pc->anim.type == ANIM_SLIDE_RIGHT) {
        ...
        render_page_content(pc, u8g2, status, pc->anim.from_page, out_offset);

        /* Render incoming page (new page sliding in) */
        render_title_bar(pc, u8g2, status, pc->anim.to_page, in_offset, now_ms);
        render_page_content(pc, u8g2, status, pc->anim.to_page, in_offset);
    } else {
        /* Normal render */
        render_title_bar(pc, u8g2, status, pc->current_page, 0, now_ms);
        render_page_content(pc, u8g2, status, pc->current_page, 0);
    }
}
