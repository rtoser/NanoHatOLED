...
static void event_loop_handle_gpio(event_loop_t *loop) {
    ...
    while (handled < MAX_GPIO_EVENTS_PER_POLL &&
        ...
        app_event_t evt = {
            ...
            .line = gpio_evt.line,
            /* Use time_hal_now_ns() instead of GPIO hardware timestamp
             * because libgpiod timestamp may use different clock source
             * (CLOCK_REALTIME) than our CLOCK_MONOTONIC based timers.
             */
